...
def run_synthetic_mode(algorithm):
    ...
    ref_noise = np.random.normal(
        0, 0.5, len(t)
    )  # generates random numbers (white noise)

    # Simulate noise hitting primary mic with a small delay
    primary_mic = clean_voice_ideal + (
        np.roll(ref_noise, 5) * 0.8
    ...
    primary_mic = primary_mic / np.max(
        np.abs(primary_mic)
    )  # make max point = 1 0 < mu < 2/(L*P_x) stability condition
    ref_noise = ref_noise / np.max(
        np.abs(ref_noise)
    )  # as numbers can be huge and multiplication will lead to higher numbers
...
def run_real_audio_mode(algorithm):
    ...
    if primary_mic.ndim > 1:
        primary_mic = primary_mic[:, 0]
    if ref_noise.ndim > 1:
        ref_noise = ref_noise[:, 0]
